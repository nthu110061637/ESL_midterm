...
{					// Stratus co-simulation hub. It should delete the top-level
	delete m_system;
	std::cout<< "Simulated time == " << sc_core::sc_time_stamp() << std::endl;		// module instance.
	sc_time clock_period(5, SC_NS);
	std::cout<< "Simulated time == " << (int)(sc_core::sc_time_stamp() / clock_period) << "cycles" << std::endl;
}
